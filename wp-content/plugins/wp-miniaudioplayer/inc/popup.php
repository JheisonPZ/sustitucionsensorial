...
//                console.debug(i, metadata[i]);
    ...
    jQuery.extend({metadata:{defaults:{type:"class",name:"metadata",cre:/({.*})/,single:"metadata"},setType:function(b,e){this.defaults.type=b;this.defaults.name=e},get:function(b,e){var c=jQuery.extend({},this.defaults,e);c.single.length||(c.single="metadata");var a=jQuery.data(b,c.single);if(a)return a;a="{}";if("class"==c.type){var d=c.cre.exec(b.className);d&&(a=d[1])}else if("elem"==c.type){if(!b.getElementsByTagName)return;d=b.getElementsByTagName(c.name);d.length&&(a=jQuery.trim(d[0].innerHTML))}else void 0!=
    b.getAttribute&&(d=b.getAttribute(c.name))&&(a=d);0>a.indexOf("{")&&(a="{"+a+"}");a=eval("("+a+")");jQuery.data(b,c.single,a);return a}}});jQuery.fn.metadata=function(b){return jQuery.metadata.get(this[0],b)};
